...
function renderFilteredTable() {
    ...
    filteredData.slice(1).forEach((row, rowIndex) => {
        ...
        visibleColumns.forEach(colIndex => {
            ...
            const cellValue = row[colIndex] != null ? row[colIndex] : 'Desconocido';
            td.textContent = cellValue;
            if (currentSearchQuery && cellValue.toLowerCase().includes(currentSearchQuery)) {
...
function updatePDFDownload() {
    ...
    window.downloadCSVTableAsPDF = function() {
        ...
        const data = originalData.slice(1)
            ...
            .map(row => visibleColumns.map(index => row[index] != null ? row[index] : 'Desconocido'));
